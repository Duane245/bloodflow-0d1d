...
% C =  0;
% % % % % % % % % % % % % % % % % % % % % % % % % % % 
Pc = zeros(1,number_of_time_step+1);
qout = zeros(1,number_of_time_step+1);

phi = R2*C/dt + R2*dt/(L_inductance+R2*dt);
...
for time_step = 1:number_of_time_step
    ...
    UL = Q(end)/A(end);
    AL = A(end);
    qout(time_step) = Q(end);
    
    Pc(time_step+1) = (R2*C/dt*Pc(time_step) + R2*AL*UL + (R2/(L_inductance+R2*dt))*(dt*pout-L_inductance*qout(time_step)))/phi;
    
    pout_RCLR = Pc(time_step+1)-R2/phi*AL*UL;
    
    F = @(X) ( (R1+R2/phi)*(( UL + 4*sqrt(beta/2/rho)*(AL^(1/4))*100) * X - 4*sqrt(beta/2/rho)*(X^(1/4))*100*X) - beta*(sqrt(X)-sqrt(A0)) + pout_RCLR);
    X0 = A(end);
    ...
    Uxing = (PAxing - pout_RCLR)/Axing/(R1+R2/phi);
