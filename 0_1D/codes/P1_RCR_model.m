...
for time_step = 1:number_of_time_step
    ...
    Pc(time_step+1) = Pc(time_step) + dt/C *(AL*UL - (Pc(time_step)-pout)/R2);
    
    pout_RCR = Pc(time_step+1);
    
    F = @(X) ( R*( UL + 4*sqrt(beta/2/rho)*(AL^(1/4))*100) * X - 4*R*sqrt(beta/2/rho)*(X^(1/4))*100*X - beta*(sqrt(X)-sqrt(A0)) + pout_RCR);
    X0 = A(end);
...
ylabel("P/Pmax")
ylim([-1,1])
title("single resistance model (R1 = Z0)")
% title("CR model")
